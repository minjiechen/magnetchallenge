...
class Vit2Norm(nn.Module):
    ...
    def update(self, minibatches, opt, sch, DA_weight=0.1):
        all_b = torch.cat([data[6].cuda().float() for data in minibatches])
        all_h = torch.cat([data[7].cuda().float() for data in minibatches])
        all_t = torch.cat([data[8].cuda().float() for data in minibatches])
        all_freq = torch.cat([data[9].cuda().float() for data in minibatches])
        all_pk = torch.cat([data[10].cuda().float() for data in minibatches])
        all_h_noize = torch.cat([data[12].cuda().float() for data in minibatches])
        ...
        all_features = torch.cat(
            # (all_kv, all_freq.unsqueeze(2).repeat(1, 64, 1), all_t.unsqueeze(2).repeat(1, 64, 1)), dim=2)
            (all_kv, all_freq.unsqueeze(2).repeat(1, 65, 1), all_t.unsqueeze(2).repeat(1, 65, 1)), dim=2)
        ...
        all_pred_h = self.vit_decoder(all_kv_p[:, 1:, :], all_h_noize)[:, :-16].unsqueeze(2)
        # 回归器更新
    ...
    # predict还需要重写
    def predict(self, b, h_noize, freq, t):
        all_kv = self.vit_encoder(b)
        all_features = torch.cat(
            (all_kv, freq.unsqueeze(2).repeat(1, 65, 1), t.unsqueeze(2).repeat(1, 65, 1)), dim=2)

        all_kv_p = self.featurizer(all_features)  # (288, 1)

        all_pred_h = self.vit_decoder(all_kv_p[:, 1:, :], h_noize)[:, :-16].unsqueeze(2)
