...
def core_loss(data_B, data_F, data_T):
    device = torch.device("cuda:0") # if no cuda, just change the device to 'cpu'
    BATCH_SIZE = 128 # if out of cuda memory, just decrease the batchsize.
    ...
    model = torch.load('./Model_E_mixN87.pth') # load model
    y_pred = []
    model.eval()
    with torch.no_grad():
