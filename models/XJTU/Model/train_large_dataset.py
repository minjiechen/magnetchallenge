...
data_path = "../data/" + "3C90_cycle"
save_figure_path = "./figure/" + "3C90"
save_model_path = "./model/" + "3C90"

seq = pd.read_csv(data_path+"/B_waveform[T].csv", header=None)
f = pd.read_csv(data_path+"/Frequency[Hz].csv", header=None)
T = pd.read_csv(data_path+"/Temperature[C].csv", header=None)
lab = pd.read_csv(data_path+"/Volumetric_losses[Wm-3].csv", header=None)
...
train_size = round(0.7*data_size)
#train_size = 1400
valid_size = round(0.2*data_size)
#valid_size = 800
test_size = round(0.1*data_size)-1
#test_size = 1000
...
#plt.plot(range(data_size),lab_normalized)
#plt.show()
...
lr_list = []
init_lr=0.01
optimizer = torch.optim.Adam(model.parameters(), lr = init_lr)  # 优化器
