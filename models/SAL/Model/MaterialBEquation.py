...
def MaterialBEquation(freq: float, temp: float, B_wf: list) -> float:
    return StoiberReynvannEquation(freq=freq, temp=temp, B_wf=B_wf, materialId=11)

def MaterialB2CSV(folder_path: str) -> float:
    freq, temp, B_wf = load_data(meas_path=folder_path)
    VLoss = np.zeros_like(freq)
    for idx, (f,t,B) in enumerate(zip(freq,temp,B_wf)):
        VLoss[idx] = MaterialBEquation(freq=f, temp=t, B_wf=B)
    VLoss.reshape((1,-1))
    np.savetxt(f'./Material B.csv', VLoss)

if __name__ == '__main__':
    MaterialB2CSV(folder_path=LIBRARY_PATH+'Material B/')