...
function main()
    ...
    [data_B, data_F, data_T, data_P] = load_dataset();
    core_loss(data_B, data_F, data_T, data_P);
...
function [data_B, data_F, data_T, data_P] = load_dataset()
    ...
    data_B = readmatrix(['./Fitting/', Material, '/B_Field.csv']); % N by 1024, in T
    data_F = readmatrix(['./Fitting/', Material, '/Frequency.csv']); % N by 1, in Hz
    data_T = readmatrix(['./Fitting/', Material, '/Temperature.csv']); % N by 1, in C
    data_P = readmatrix(['./Fitting/', Material, '/Volumetric_Loss.csv']); % N by 1, in C

end

%% Calculate Core Loss
function core_loss(data_B, data_F, data_T, data_P)
...
% ESTIMATE LOSSES --- ESTIMATEA LOSSES --- ESTIMATEA LOSSES --- ESTIMATEA LOSSES --- ESTIMATEA LOSSES --- ESTIMATEA LOSSES --- ESTIMATEA LOSSES --- ESTIMATEA LOSSES --- ESTIMATEA LOSSES --- ESTIMATEA LOSSES
    ...
    E=abs(data_P-data_Pe)./data_P*100;
    mean(E)
    prctile(E,95)
    prctile(E,99)
    max(E)
    writematrix(data_Pe, ['./Fitting/Volumetric_Loss_', Material, '.csv']);
